$(document).ready(function(){
    ...
    $("#input-searchbox").keyup(function(){
        ...
        var expr = new RegExp(searchField, "igm"); // dung regex search i khong phan biet chu cai
        $.getJSON("../db/datasearchbox.json", function(datasearchbox){
            $.each(datasearchbox, function(index, value){
                if (value.name.search(expr) != -1)
                {
                    //$('#result-searchbox').append('<li class="list-group-item link-class"><img src="'+value.image+'" height="40" width="40" class="img-thumbnail" /> '+value.name+' | <span class="text-muted">'+val.category+'</span><a target="_blank" href="'+val.link + '"><img class"navbar-link" src="../data/chevron-right-regular-24.png" alt="link"></a></li>');
